  ...
  if (isDevelopment && isTestSignature) {
    ...
    try {
      ...
      if (req.body && typeof req.body === 'object' && (req.body as any).type) {
        ...
        // RAILWAY FALLBACK: Poll Stripe when body is completely missing
        console.log("[Stripe Webhook] ⚠️ RAILWAY ISSUE: No webhook body available, polling Stripe for recent payments...")

        const stripeKeyForPolling = process.env.STRIPE_API_KEY
        if (!stripeKeyForPolling) {
          return res.status(500).json({ error: "Stripe not configured for polling" })
        }

        const stripePolling = new Stripe(stripeKeyForPolling, {
          apiVersion: '2025-08-27.basil',
        })
        ...
        try {
          ...
          const events = await stripePolling.events.list({
            limit: 10,
          ...
          for (const eventItem of events.data) {
            const paymentIntent = eventItem.data.object as Stripe.PaymentIntent
            ...
            if (!existingOrders || existingOrders.length === 0) {
              ...
              event = {
                id: eventItem.id,
                object: 'event',
                api_version: eventItem.api_version,
                created: eventItem.created,
                data: { object: paymentIntent },
                livemode: eventItem.livemode,
                pending_webhooks: 0,
                request: eventItem.request,
                type: 'payment_intent.succeeded'
              } as Stripe.Event

              // Break to process this event
              break
            }
          }

          // If no unprocessed events found
          if (!event) {
            return res.json({
              received: true,
