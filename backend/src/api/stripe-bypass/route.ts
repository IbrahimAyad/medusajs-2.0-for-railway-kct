...
export async function POST(req: MedusaRequest, res: MedusaResponse) {
  ...
  try {
    ...
    // DEBUG: Log the actual item data to see what we're receiving
    console.log("[DEBUG] Items received:", JSON.stringify(items?.map(item => ({
      title: item.title,
      variant: item.variant,
      variant_title: item.variant?.title,
      has_variant: !!item.variant,
      variant_keys: item.variant ? Object.keys(item.variant) : []
    })), null, 2))
